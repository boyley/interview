...
> **核心理念：Redis 是内存数据库，避免内存耗尽 = 「限上限 + 勤清理 + 省着用 + 防大key + 监控」多管齐下。**
> 答题方法：先给分层思路，再逐步展开——设上限 → 淘汰策略 → 过期删除 → 省内存 → 防碎片/大key → 监控扩容。

## 步骤 1 · 设内存上限（maxmemory，第一道闸）
- 配置 `maxmemory`，别让它无限吃内存（默认 0 = 不限制，64 位下很危险）
- 建议设为物理内存的 **70%~75%**，给持久化 fork 的 COW、系统本身留余量

## 步骤 2 · 选对内存淘汰策略（maxmemory-policy，★ 达上限后淘汰谁）

| 策略 | 含义 |
|---|---|
| **noeviction**(默认) | 不淘汰，写操作直接报错（读正常） |
| **allkeys-lru / volatile-lru** | 所有 key / 仅带过期 key 中，淘汰最久未用 |
| **allkeys-lfu / volatile-lfu** | 淘汰访问频率最低（解决"历史热、近期冷"缓存污染） |
| **allkeys-random / volatile-random** | 随机淘汰 |
| **volatile-ttl** | 带过期 key 中，淘汰剩余 TTL 最短的 |

- 生产常用 **allkeys-lru** 或 **allkeys-lfu**
- 原理：**近似 LRU/LFU**，不维护全局链表（省内存），淘汰时**随机采样** `maxmemory-samples` 个 key 挑最该淘汰的

## 步骤 3 · 过期删除（给 key 设 TTL，主动清理）
- **一定给能过期的 key 设 TTL**，别让 key 只进不出
- 删除机制 = **惰性删除**（访问时才删）+ **定期删除**（周期性随机抽查删），配合淘汰策略兜底

## 步骤 4 · 省着用（数据结构 / 编码优化）
- 用合适**编码**：小 Hash/ZSet 用 listpack、纯整数 Set 用 intset；控制元素数量/大小，别让它升级成 hashtable/skiplist
- 用 **Hash 存对象** 比拆成多个 String 省内存（共享 key 开销）
- 短 key 名、合理数据结构、控制 value 大小

## 步骤 5 · 防大 key + 内存碎片
- **大 key**：拆分；删除用 **`unlink`**（异步删）代替 `del`，避免阻塞
- **内存碎片**：`activedefrag yes` 自动碎片整理；看 `mem_fragmentation_ratio`
- **持久化**：`bgsave`/AOF 重写 fork 时 COW 可能使内存接近翻倍，务必留余量

## 步骤 6 · 监控 + 扩容
- `INFO memory` 看：`used_memory`、`maxmemory`、`mem_fragmentation_ratio`、`evicted_keys`
- 监控 **内存使用率、evicted_keys(淘汰量)、内存增长趋势**，设告警
- 顶不住就**横向扩展**：Redis Cluster 分片，把数据摊到多节点

---

> **核心原则：设上限(maxmemory) + 选对淘汰策略(allkeys-lru/lfu) + 全设 TTL + 数据结构省内存 + 拆大key防碎片 + 监控告警 + 分片扩容。**

---

# 📏 专题：存储 7000 万条数据，Redis 怎么预估内存占用？

> **核心理念：内存预估 = 选对结构 → 算「单条实际占用」× 条数 + 结构开销 + 碎片预留。单条 ≠ value 本身，还要加 Redis 的对象/哈希表/分配器开销；最靠谱是「抽样实测外推」。**
> 答题方法：先给预估方法论 → 拆单条隐藏开销 → 讲结构差异 → 举例算 7000 万 → 实测校准。

## 一、预估方法论（大步骤）
1. **先定数据结构**（String / Hash / ZSet…）—— 结构不同，占用差几倍
2. **算单条实际占用**（key + value + 各种隐藏开销）
3. **× 条数 + 结构总开销**，再 **× 碎片系数(~1.5)**
4. **抽样实测校准**（理论估不准，实测最靠谱）
...
| 开销项 | 大小 |
|---|---|
| **dictEntry**（全局哈希表条目） | ~24 字节（3 指针） |
| **key 的 SDS** | key 长度 + SDS 头 + `\0` |
| **redisObject**（每个 value 一个 robj） | ~16 字节（type/encoding/lru/refcount/ptr） |
| **value 的 SDS / 编码** | value 长度 + 头 |
| **jemalloc 向上取整** | 分配按 2 的幂档位，有内部碎片 |

> 经验：**一个简单 String kv，哪怕 value 很短，也要 ~60~90 字节起**。

## 三、选对结构省一半（★ 关键）

| 存法 | 单条开销 | 说明 |
|---|---|---|
| **String**（一对象一 key） | 最大 | 每 key 都有 dictEntry + robj |
| **Hash 分桶 + listpack** | 最小 | field 少(<128)、值小(<64B)时用 listpack，**无 per-field 的 robj/dictEntry**，极紧凑 |

> 结论：**7000 万个对象，用 Hash 分桶（如 100 个对象塞一个 Hash、走 listpack）比 7000 万个 String 省好几倍。**
...
| 方案 | 单条 | 总量(×7000万) | +碎片 ≈ |
|---|---|---|---|
| A. String + JSON | ~100B | ~7GB | **~10GB** |
| B. Hash 分桶(listpack) | ~40B | ~2.8GB | **~4GB** |

> 公式：**总内存 ≈ 条数 × 单条估算 × (1 + 碎片率 ~0.5)**

## 五、怎么"预估"最靠谱（小步骤）
1. **理论估**：按结构算单条 × 条数
2. **★ 抽样实测（最准）**：写 1~10 万条**真实数据** → `INFO memory` 看 `used_memory` ÷ 条数 = 单条均值 → × 7000 万
3. `MEMORY USAGE key` 看单个 key 实际字节
4. `redis-cli --bigkeys` 找大 key
5. **留 buffer**：碎片(`mem_fragmentation_ratio`)、主从复制缓冲、AOF/RDB fork 的 COW

## 六、顺带优化
- Hash + listpack 分桶；短 key；int 编码；拆冷热数据
- 单实例别太大（建议 < 10~20GB，利于持久化/迁移）→ **Cluster 分片**

---

> **核心原则：内存预估 = 选对结构 → 单条(key+value+robj+dictEntry+SDS+分配器取整) × 条数 + 碎片预留；最靠谱是「写 N 条看 used_memory 外推」；7000 万条优先 Hash+listpack 分桶 + 集群分片。**
